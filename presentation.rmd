...
graph_no_isolated_nodes <- graph %>%
        activate(nodes) %>%
        mutate(influence = centrality_degree(mode = 'in')) %>%
        filter(!node_is_isolated()) %>%
        filter(influence > 0)
...
clustered <- graph_no_isolated_nodes %>%
        activate(nodes) %>%
        filter(!node_is_isolated()) %>%
        mutate(group = group_infomap() %>%  as.factor())
clustered %>% as_data_frame(what = 'vertices') %>% as_tibble() %>% select(group) %>% distinct() %>% count()
...
clusters <- fastgreedy.community(graph_no_isolated_nodes)
weights <- ifelse(crossing(clusters, graph_no_isolated_nodes), 1 , 100)
plot(graph_no_isolated_nodes, layout = layout_with_kk(graph_no_isolated_nodes, weights = weights))
```
